...
=head1 HOW TO ADD HTTP Session SUPPORT
...
配列リファレンスの1つ目の引数はHTTP::Session::State::*の名前を指定します。(storeの場合はHTTP::Session::Store::*)
2つ目の引数は使用するモジュールをnewする時の引数です。

必要な引数については、HTTP::Session::State::*,HTTP::Session::Store::*の各ドキュメントを
参照してください。
